...
Logits are the **raw, unnormalized output scores** of a neural network before any activation function like [[List of Layers#Softmax|softmax]] is applied. They are called "logits" because they live in **log-odds space** — the inverse of the sigmoid/softmax.
...
- **Relationship to probability**: For a binary classification, applying the [[List of Layers#Sigmoid|sigmoid]] function $\sigma(x) = \frac{1}{1 + e^{-x}}$ converts a logit to a probability. For multi-class, [[List of Layers#Softmax|softmax]] is used instead.
...
3. **Gradient flow**: The [[List of Layers#Softmax|softmax]] + cross-entropy combination has clean gradients: $\frac{\partial L}{\partial z_i} = p_i - y_i$ (the gradient is simply `probs - targets`).