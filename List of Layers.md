...
**In makemore_part3_bn.ipynb:** `C = torch.randn((vocab_size, n_embd))` — one row per character (27), 10 dimensions each. `emb = C[X]` grabs the embedding of every context character; `emb.view(emb.shape[0], -1)` concatenates the block of 3 into one 30-dim vector.
...
**In makemore_part3_bn.ipynb:** `W1` (`30 → 200`) and `W2` (`200 → 27`), later wrapped in a `Linear` class initialized with fan-in normalization: `torch.randn((fan_in, fan_out)) / fan_in**0.5` (see Kaiming init in [[Activations & Gradients, BatchNorm in MLP]]).
...
**In makemore_part3_bn.ipynb:** first implemented manually (`bnmeani`, `bnvar`, `bngain`, `bnbias`, `bnmean_running`/`bnstd_running`), then wrapped as a `BatchNorm1d` class with `gamma`/`beta` parameters and `running_mean`/`running_var` buffers. The last layer's `gamma` is scaled by `0.1` to make the final logits less confident.
...
**In makemore_part3_bn.ipynb:** `h = torch.tanh(hpreact)`, wrapped in a `Tanh` class. The earlier sections of the notebook show exactly what goes wrong when pre-activations are too broad (saturated, dead neurons) — see [[Activations & Gradients, BatchNorm in MLP]].
...
**In makemore_part3_bn.ipynb:** implicit in `F.cross_entropy(logits, y)` during training; explicit `F.softmax(logits, dim=1)` when sampling names.
